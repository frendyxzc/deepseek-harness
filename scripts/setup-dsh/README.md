...
5. **Refreshes the better-harness skill** — fetches and resets the checkout at
   `~/.dsh/better-harness` to its pinned branch and reinstalls its root runtime
   deps, keeping the `/better-harness` workflow-review skill current (see the
   [better-harness skill](#better-harness-skill-optional) section).
...
| `~/.dsh/better-harness/` | git clone `main` of [QoderAI/better-harness](https://github.com/QoderAI/better-harness) + `npm ci` (root runtime deps) | no |
| `~/.dsh/skills/better-harness/` | wrapper SKILL.md delegating to the checkout (generated by §8) | no |
...
Installed by default (`--skip-better-harness` turns it off): setup.sh clones the
open-source [QoderAI/better-harness](https://github.com/QoderAI/better-harness)
repo to `~/.dsh/better-harness`, installs its root runtime dependencies under
the pinned Node v22, and writes a small wrapper skill to
`~/.dsh/skills/better-harness/` — the user skills root `skill-filesystem` scans
— that points the agent at the checkout's canonical SKILL.md. In any session
...
better-harness reads workspace-matched local session logs (bounded, read-only,
redacted) plus the project's static harness evidence, and produces a
five-dimension Agent Work Loop report with prioritized findings.
`setup.sh --upgrade` refreshes the checkout and its deps; the wrapper skill is
kept on re-run (rewritten only with `--force`).

Relevant vars: `DSH_BETTER_HARNESS_REPO_URL` (default the public
QoderAI/better-harness repo), `DSH_BETTER_HARNESS_BRANCH` (default `main`).
