...
4. `@deepseek-ai/dsh-feishu-receive`（`packages/feishu/feishu-receive`）把收到的飞书消息作为用户消息桥接进活跃 agent 会话。
...
### 入站投递唤醒第一个 root agent

`dsh-feishu-receive` 在 `ctx.effect` 内启动接收通道（其 disposer 会关闭长连接），并通过 `agents.roots()[0].followup(createUserMessage(...))` 投递每条 `FeishuReceiveEvent`。`followup` 会唤醒空闲的 agent，因此当没有 agent 正在运行时消息也不会被静默丢弃——早先「只有运行中的 agent 才接收」是错误的路由判据。提供方只提取内容解码后非空的文本消息；其他消息类型被忽略。
...
- 入站路由是单 agent（第一个 root agent）；按聊天/发送者的定向路由与过滤被推迟，记录在 `dsh-feishu-receive` 的 README 中。
