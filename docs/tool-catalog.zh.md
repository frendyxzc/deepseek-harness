...
| `@deepseek-ai/dsh-tool-feishu` | `feishu_send_message` | `ctx.tools`、`ctx.feishu`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | feishu_send_message 将提供方选择置于 ctx.feishu 之后，使模型可见 schema 在更换后端时保持稳定。 |
