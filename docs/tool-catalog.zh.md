...
| --- | --- | --- | --- | --- | --- |
| `@deepseek-ai/dsh-tool-ask-user` | `ask_user_question` | `ctx.tools`、`ctx.userQuestions` | `tool/call`、`tool/result after a UI/provider answers the question` | - | ask_user_question 会暂停工具调用，直到某个 UI 提供方返回人类答案。 |
...
ask_user_question 会暂停工具调用，直到某个 UI 提供方返回人类答案。
...
通过飞书（Feishu）聊天发送消息。需要有效的接收方 id（open_id、user_id 或 chat_id）以及消息内容。
...
更新此前发送的一条飞书（Feishu）消息的内容。需要 feishu_send_message 返回的消息 id 与替换内容。
