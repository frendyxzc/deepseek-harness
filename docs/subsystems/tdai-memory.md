# TDAI Memory Identity

English | [中文](tdai-memory.zh.md)

The TDAI MemoryProxy team/agent identity seam — the `ctx.tdaiMemory` service that resolves outgoing LLM requests to a Feishu bot's team/agent headers and binds them per agent session. The per-bot mapping lives in the `feishu-bot` settings section (`bots[].teamId` / `bots[].agentId`); this seam owns the runtime coordination: `feishu-receive` writes the session → bot bindings, `tdaiMemory` turns a bound session back into the team/agent headers the `llm-pi-ai` / `llm-deepseek` adapters send, and `listTeams` / `listAgents` are Typert Remotes so the configuration UI can offer dropdowns. Every bound session also carries the default task as `x-task-id` (see `defaultTaskId`).

Source: [`packages/llm/tdai-memory/src/index.ts`](../../packages/llm/tdai-memory/src/index.ts)

## Session binding

A Feishu message received by bot `X` binds its per-chat session to `X`; every LLM request from that session then carries the bot's team/agent headers plus the default task's `x-task-id`. Sessions not bound to any bot (plain Web/harness sessions) send no identity headers. The header mapping is the pure function `tdaiMemoryHeaders`: empty/absent ids are omitted, so an unpinned bot sends nothing.
