...
- `ctx.feishu.listProviders()` — every registered provider, in registration order.
- `ctx.feishu.sendMessage(request, signal?)` — send one message. Routes through the request's explicit `providerId`, else the provider that last delivered `request.receiveId`, else the selection rules; a missing routed provider throws `FEISHU_PROVIDER_CONFIGURED_MISSING`.
...
- `ctx.feishu.startReceivingAll(handler)` — start every available provider that can receive; each event is stamped with its provider id and recorded against its chat so replies route back through the same app. Returns a disposer closing every opened channel; throws `FEISHU_RECEIVE_UNSUPPORTED` when an available provider cannot receive and none can.
...
- `ctx.feishu.getMessageResource(messageId, fileKey, signal?)` — fetch the raw bytes of one image attached to a message by its file key (a `FeishuMessageImage.fileKey`) so a multimodal model can read it; throws `FEISHU_RESOURCE_UNSUPPORTED` when the provider has no resource-read support.
