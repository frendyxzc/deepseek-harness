...
每条入站聊天消息还会在按聊天划分的 agent 启动前，先向该聊天发送一条简短确认（"已收到，正在处理…"），让用户立即得知消息已到达。确认发送失败仅记入日志，绝不阻塞投递。

当入站消息引用或回复另一条消息（其 `parentId` / `rootId`）时，该消费方通过 `ctx.feishu.getMessage` 读取被引用的消息，并把其文本以 `[引用消息]` 块的形式前置到回复之前，让 agent 看到完整上下文。缺失 `getMessage` 的提供方，或没有可读内容的被引用消息，都会让回复保持原样，绝不阻塞投递。

入站或被引用消息中的图片（其 `images`）会通过 `ctx.feishu.getMessageResource` 下载，保存到挂载的 `ctx.attachments`，并作为面向模型的 `image` 内容块附加到文本之后。被删除或无法识别的图片会被跳过并记入日志——读取图片绝不阻塞投递，纯文本回退仍会带上图片标记。
...
| `ack` | `boolean` | `true` | 在按聊天划分的 agent 启动前，对每条入站聊天消息回复一条简短确认。发送失败仅记入日志，绝不阻塞投递。 |
...
- **图片可用性取决于飞书留存** —— 消息图片可能在再次读取前已被飞书侧删除；此时该消费方回退到纯文本标记，而非图片。