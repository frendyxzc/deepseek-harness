...
The plugin fails loud at load when the selected Feishu provider cannot receive card actions — answering is impossible without the tap channel.
...
- **Card taps ride the provider's receive channel** — a provider without `startReceivingCardActions` cannot host this plugin (fails loud at load); the long-connection provider shares ONE connection between message and card-action subscribers.
