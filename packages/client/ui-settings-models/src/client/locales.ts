...
export const en = {
  ...
  loopbackOnly: 'Model configuration is local-only: settings and credentials cannot be read or changed over the LAN. Open this page in a browser on the host machine, or use an SSH tunnel (ssh -L 3080:127.0.0.1:3080 and visit http://localhost:3080).',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  loopbackOnly: '模型配置仅限本机访问：通过局域网无法读取或修改设置与凭据。请在本机浏览器打开，或使用 SSH 隧道（ssh -L 3080:127.0.0.1:3080 后访问 http://localhost:3080）。',
